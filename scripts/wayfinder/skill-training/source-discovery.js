...
function selectionPersistence(source, sourceRuleIndex) {
...
function discoverDescriptionTrainingMeta(args) {
    const descriptionText = normalizeDescriptionText(args.source.sourceDocument?.system?.description?.value);
    if (!descriptionText) {
        return {
            fixedLores: [],
            choiceRules: [],
    ...
    if (choiceRules.length === 0) {
        ...
        for (const [index, match] of genericSkillMatches.entries()) {
            ...
            const abilities = match[2]
                ? Array.from(new Set(match[2]
                    .split(/-\s*or\s*/i)
                    .map((entry) => abilityWordToKey(entry))
                    .filter((entry) => typeof entry === "string")))
                : [];
            const prompt = abilities.length > 0
    ...
    if (loreChoices.length === 0) {
        const contextualLoreMatch = /\ba lore skill (?:associated with|related to|specializing in)\s+([^.;]+)/i.exec(descriptionText);
...
function buildFilteredSkillOptions(localize, configuredSkills, allowedAbilities) {
    const skillEntries = Object.keys(configuredSkills).length > 0 ? Object.keys(configuredSkills) : Object.keys(SKILL_LABELS);
    return skillEntries
        .filter((slug) => allowedAbilities.length === 0 || allowedAbilities.includes(getSkillAbility(slug, configuredSkills) ?? ""))
        .map((slug) => ({
        slug,
...
function normalizeDescriptionText(value) {
    ...
    return value
        .replace(/@UUID\[[^\]]*\.Item\.([^|\]]+)(?:\|[^\]]+)?\](?:\{([^}]+)\})?/gi, (_match, fallbackLabel, explicitLabel) => explicitLabel ?? fallbackLabel)
        .replace(/<[^>]+>/g, " ")
        .replace(/&nbsp;/gi, " ")
        .replace(/\s+/g, " ")
        .trim();
...
function abilityWordToKey(value) {
    switch (value.trim().toLowerCase()) {
        case "strength":
            return "str";
        case "dexterity":
...
function topicToLoreLabel(topic) {
...
function discoverAdditionalLoreGrantMeta(args) {
    const fixedLores = [];
    const loreChoices = [];
    const matches = Array.from(args.descriptionText.matchAll(/\badditional lore\b(?:\s+(?:general|skill)\s+feat|\s+feat)?(?:\s+and\s+[^.;]+?)?\s+(for|in)\s+([^.;]+)/gi));
    for (const [index, match] of matches.entries()) {
        const relation = (match[1] ?? "").toLowerCase();
        const clause = (match[2] ?? "").trim();
        const normalizedClause = clause.replace(/\s+/g, " ");
        const loreLabels = Array.from(new Set(Array.from(normalizedClause.matchAll(/\b([A-Za-z][A-Za-z' -]*? Lore)\b/gi))
        ...
        const explicitSubcategoryMatch = /\b(?:a\s+special\s+)?lore skill subcategory\s*[—:-]\s*([A-Za-z][A-Za-z' -]+ Lore)\b/i.exec(normalizedClause);
        ...
        const isPureAlternativeChoice = loreLabels.length > 1 &&
...
function additionalLorePlaceholder(clause, loreLabels) {
