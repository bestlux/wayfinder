...
const CLASS_BOUNDARIES = {
    animist: "Wayfinder cannot confidently audit animist spell totals because apparition spells are not fully represented by its spell-choice builder.",
    cleric: "Wayfinder does not audit cleric daily preparation slots, and clerics do not have a finite spellbook or repertoire total to compare.",
    druid: "Wayfinder does not audit druid daily preparation slots, and druids do not have a finite spellbook or repertoire total to compare.",
    psychic: "Wayfinder cannot confidently audit psychic spell totals because psi cantrips and conscious-mind grants are not fully represented by its spell-choice builder.",
    summoner: "Wayfinder cannot confidently audit summoner spell totals because bounded repertoire replacement is not fully represented by its spell-choice builder.",
};
export async function buildExistingCharacterSpellAuditEntries(actor, actorLevel) {
    ...
    if (!AUDITED_CLASS_SLUGS.has(classSlug)) {
        return Number(classDocument?.system?.spellcasting) > 0
            ? [
                boundaryEntry(classSlug, className(classDocument, classSlug), actorLevel, "Wayfinder's current spell-choice builders do not model this caster profile confidently enough for a numeric audit."),
            ]
            : [];
    }
...
function resolveRequiredClassChoices(classSlug, classFeatures, actorLevel) {
    if (classSlug === "witch") {
        ...
        if (!strictTraditionFromClassFeature(patron)) {
            return unresolved("Wayfinder cannot resolve the witch spell total because the selected patron's tradition is unavailable.");
        }
        if (parseWitchPatronLessonSpellAccess(patron).uuids.length === 0) {
            return unresolved("Wayfinder cannot resolve the witch spell total because the selected patron's initial-lesson spell is unavailable.");
        }
    }
    if (classSlug === "wizard") {
        ...
        return school
            ...
            : unresolved("Wayfinder cannot resolve the wizard spellbook total because no arcane school is selected on the actor.");
    }
    if (classSlug === "sorcerer") {
        const branch = findClassFeatureDocumentByOtherTag(classFeatures, "sorcerer-bloodline");
        if (!branch) {
            return unresolved("Wayfinder cannot resolve the sorcerer repertoire total because no bloodline is selected on the actor.");
        }
...
function strictTraditionFromClassFeature(document) {
...
function boundaryEntry(classSlug, classLabel, actorLevel, boundary) {
    return {
        slotId: `spell-audit-${classSlug}-through-level-${actorLevel}`,
        level: actorLevel,
        category: "other",
...
function className(classDocument, classSlug) {
...
//# sourceMappingURL=existing-character-spell-audit-service.js.map