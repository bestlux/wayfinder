...
interface AcquisitionSmokeCaseDefinition {
  ...
  readonly acquisitionCase: {
    readonly schemaVersion: 1;
    readonly executorRole: "non-gm-owner";
    readonly targetLevel: 1;
...
async function exposePartialItemOnActorSheet(
  ...
  const row = await waitForValue(
    () => actorSheetRootOf(actor)?.querySelector<HTMLElement>(`[data-inventory] [data-item-id="${itemId}"]`) ?? null,
    "partially created PF2E inventory row"
  );
...
function assertCurrentPlayerAndRuntime(actor: SmokeActor, marker: AcquisitionSmokeMarker): void {
