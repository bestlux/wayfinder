...
export const COMPENDIUM_COUNT_FIELDS = ["type", "system.category", "system.featType.value"] as const;

export interface CompendiumContentCounts {
  totalItems: number;
...
export interface CompendiumCatalogRow {
  id: string;
  title: string;
  packageId: string;
  packageTitle: string;
  ...
  counts: CompendiumContentCounts;
}

export interface CompendiumSelectionMigration {
  selectedIds: string[];
  unavailableExactIds: string[];
  legacyPatterns: string[];
  unmatchedLegacyPatterns: string[];
  hasGlobalWildcard: boolean;
}

interface CompendiumIndexEntryLike {
  type?: unknown;
  system?: {
    category?: unknown;
    featType?: {
...
export interface ItemCompendiumLike {
  ...
  getIndex(options: { fields: string[] }): Promise<Iterable<CompendiumIndexEntryLike> | null | undefined>;
}
...
interface CatalogGlobalsLike {
  game?: {
    modules?: Map<string, PackageLike>;
    packs?: Map<string, ItemCompendiumLike>;
    system?: {
      id?: unknown;
      title?: unknown;
    };
...
const OFFICIAL_PACK_IDS = new Set<string>(Object.values(OFFICIAL_PACKS).flat());

export function emptyCompendiumContentCounts(): CompendiumContentCounts {
  return {
    totalItems: 0,
    relevantTotal: 0,
    ancestry: 0,
...
export function classifyCompendiumEntries(entries: Iterable<CompendiumIndexEntryLike>): CompendiumContentCounts {
  ...
  for (const entry of entries) {
    ...
    switch (type) {
      ...
      case "feat": {
        counts.feats += 1;
        const featType =
          normalizedIdentifier(entry.system?.category) ?? normalizedIdentifier(entry.system?.featType?.value);
        if (featType === "ancestry") counts.ancestryFeats += 1;
        if (featType === "class" || featType === "archetype") counts.classFeats += 1;
        if (featType === "skill") counts.skillFeats += 1;
        if (featType === "general") counts.generalFeats += 1;
        if (featType === "classfeature") counts.classFeatures += 1;
        break;
      }
    }
  }
...
export function discoverItemCompendia(): CompendiumCatalogRow[] {
  const globals = globalThis as CatalogGlobalsLike;
...
export async function scanCompendiumCatalog(
  rows: CompendiumCatalogRow[],
  concurrency = 5
): Promise<CompendiumCatalogRow[]> {
  const globals = globalThis as CatalogGlobalsLike;
...
export function resolveCompendiumSelection(raw: unknown, availablePackIds: string[]): CompendiumSelectionMigration {
  ...
  return {
    ...
    legacyPatterns,
    unmatchedLegacyPatterns: legacyPatterns.filter(
      (pattern) => expandCompendiumAllowlist([pattern], availablePackIds).length === 0
    ),
    hasGlobalWildcard: legacyPatterns.includes("*"),
  };
}

export function serializeCompendiumSelection(
  ids: Iterable<string>,
  preservedLegacyPatterns: Iterable<string> = []
): string {
...
export function findUnavailableSelectedCompendiumIds(
  selectedIds: Iterable<string>,
  availablePackIds: Iterable<string>
): string[] {
  const available = new Set(availablePackIds);
...
export function isOfficialCompendium(packId: string): boolean {
  return OFFICIAL_PACK_IDS.has(packId);
}

function isItemCompendium(pack: ItemCompendiumLike): boolean {
...
function packageTitle(packageId: string, globals: CatalogGlobalsLike): string {
...
function compareCatalogRows(left: CompendiumCatalogRow, right: CompendiumCatalogRow): number {
