...
export class WayfinderApp extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(): Promise<any> {
    ...
    const stepStateRows = await Promise.all(
      plan.steps.map(
        async (step, index): Promise<StepNavRow> => ({
          id: step.id,
          index: index + 1,
          level: step.level,
          title: step.title,
          ...
          invalidated:
            this.#recentlyInvalidatedStepIds.has(step.slotId) &&
    ...
    return {
      ...
      guidance:
        "Review one decision at a time, keep the draft coherent, and let earlier choices narrow what comes next.",
      summary,
    ...
    switch (section) {
      ...
      case "level-20": {
        ...
        const selected = draft.boosts.levels[level] ?? [
          ...effectiveBuildState.levelBoosts[Number(level) as BoostLevel],
        ];
  ...
  async #resolveDraftOrActorArcaneSchoolDocument(): Promise<any | null> {
    const draftSelection = Object.values(this.#requireDraft().branchSelections).find((selection) =>
      /^class-branch-arcane-school-level-\d+$/.test(selection.slotId)
    );
    if (draftSelection) {
  ...
  #findDraftSelectionByType(itemType: "ancestry" | "heritage" | "background" | "class" | "deity"): SelectionRef | null {
  ...
  #invalidateClassChoicesByPrefix(prefix: string): string[] {
    const invalidated: string[] = [];
  ...
  #invalidateSpellChoicesByPrefix(prefix: string): string[] {
    const invalidated: string[] = [];
