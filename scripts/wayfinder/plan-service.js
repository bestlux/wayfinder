...
export async function buildWayfinderPlan(snapshot, draft, deps) {
    ...
    const importedHistorySuppressesEquipment = snapshot.hasImportedExistingCharacterHistory &&
        draft.acquisition === null &&
        !draft.acquisitionCorrupt &&
        draft.equipmentPolicyRequests.length === 0;
    const targetLevelSteps = !importedHistorySuppressesEquipment &&
        !snapshot.hasValidCompletedAcquisitionManifest &&
        !snapshot.fulfilledStepIds.includes(equipmentSlotId)
