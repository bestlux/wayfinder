  ...
  async project(context: EquipmentCatalogueContext): Promise<EquipmentCatalogueProjection> {
    ...
    if (cachedEntries === undefined) {
      ...
      entries = await profileEquipmentStage(
        ...
        async () => {
          const sharedAuthority = context.policy.gmJudgments.some(
            (judgment) => judgment.kind === "rarity-source-exception"
          )
            ? null
      ...
      requests.map(async ({ sourceUuid, pack, packId, documentId }) => {
        try {
          ...
          const canReusePreview =
            !forceFresh && candidate !== undefined && cachedPreview?.previewIdentity === candidate.previewIdentity;
...
async function loadPackProjection(
  ...
  try {
    index = await profileEquipmentStage(
      "catalogue-index-wait",
      () => pack.getIndex({ fields: [...INDEX_FIELDS] }),
      () => ({ packId, requestedFieldCount: INDEX_FIELDS.length })
    );
  } catch {
