import { PHYSICAL_GRANT_SCANNER_ROUTE_DISPOSITIONS, } from "./physical-grant-scanner-route-dispositions.js";
export const PHYSICAL_GRANT_COVERAGE_PF2E_VERSION = "8.4.1";
export { PHYSICAL_GRANT_SCANNER_ROUTE_DISPOSITIONS, } from "./physical-grant-scanner-route-dispositions.js";
...
const background = (id) => `Compendium.pf2e.backgrounds.Item.${id}`;
const classFeature = (id) => `Compendium.pf2e.classfeatures.Item.${id}`;
const equipment = (id) => `Compendium.pf2e.equipment-srd.Item.${id}`;
const feat = (id) => `Compendium.pf2e.feats-srd.Item.${id}`;
const heritage = (id) => `Compendium.pf2e.heritages.Item.${id}`;
...
const UNPROFILED_NATIVE_DETAIL = "PF2E creates this physical item before acquisition, but Wayfinder has no reviewed reconciliation profile for it.";
const DYNAMIC_CHOICE_DETAIL = "This build reaches a PF2E physical-item choice that Wayfinder does not capture as reviewed starting-equipment authority.";
const OWNED_OR_GRANTED_DETAIL = "This feature can grant a new physical item or depend on an owned item, and neither path is represented by reviewed Wayfinder equipment authority.";
const U = CLASS_GRANT_PROFILE_UUIDS;
export const SUPPORTED_PHYSICAL_GRANT_ROUTES = Object.freeze([
    supportedRoute({
...
export const UNSUPPORTED_PHYSICAL_GRANT_ROUTES = Object.freeze([
    selectedRoute({
    ...
    ].map(([routeId, label, featureId, terminalId]) => selectedRoute({
        routeId: routeId,
        label: label,
    ...
    ].map(([routeId, label, featureId]) => selectedRoute({
        routeId: routeId,
        label: label,
        sourceUuid: classFeature(featureId),
        reasonCode: "owned-item-dependency",
    ...
    ...[
        ...
        detail: "This physical ikon has multiple no-flag PF2E choices that collapse to one Wayfinder draft slot, so its terminal item is not reviewed deterministically.",
    })),
    selectedRoute({
    ...
    selectedRoute({
        ...
        detail: "The formula book is described in prose without an exact physical GrantItem terminal, so Wayfinder cannot assign acquisition authority to it.",
    }),
    selectedRoute({
    ...
    selectedRoute({
        ...
        detail: "Clan Pistol replaces the default Clan Dagger, but Wayfinder does not yet capture and reconcile that replacement branch.",
    }),
    selectedRoute({
        routeId: "tough-skin",
        label: "Tough Skin",
        ...
        detail: "Wayfinder does not yet prove Tough Skin's required Lethoci or Xyloshi heritage before PF2E creates its armor.",
    }),
    selectedRoute({
        routeId: "orc-warmask",
        label: "Orc Warmask",
        ...
        detail: "The granted warmask owns a nested tradition choice that Wayfinder does not capture before PF2E creates the item.",
    }),
    selectedRoute({
        ...
        detail: "Wayfinder does not yet prove the feat's religion and follower requirements before PF2E creates the free token.",
    }),
    selectedRoute({
    ...
    ].map(([routeId, label, backgroundId]) => selectedRoute({
        routeId: routeId,
        label: label,
        sourceUuid: background(backgroundId),
        reasonCode: "semantic-prerequisite-mismatch",
        terminals: [equipment("gwP3Uums2ApH6o9K")],
        detail: "This background embeds Pilgrim's Token, whose religion and follower requirements are not yet proven by Wayfinder.",
    })),
    ...[
    ...
    ].map(([routeId, label, heritageId, terminalId]) => selectedRoute({
        routeId: routeId,
        label: label,
    ...
    })),
    {
        ...
        blocker: {
            preReview: true,
            reasonCode: "unsupported-physical-choice",
            detail: "The Ancient Elf dedication path reaches Giant Instinct without the reviewed Titan Mauler acquisition profile used by a Barbarian class selection.",
        },
    },
    {
    ...
    .sort((left, right) => left.routeId.localeCompare(right.routeId)));
export const UNSUPPORTED_PHYSICAL_GRANT_ROUTE_IDS = Object.freeze(UNSUPPORTED_PHYSICAL_GRANT_ROUTES.map((route) => route.routeId));
export const PHYSICAL_GRANT_ROUTE_REGISTRY = Object.freeze([...SUPPORTED_PHYSICAL_GRANT_ROUTES, ...UNSUPPORTED_PHYSICAL_GRANT_ROUTES].sort((left, right) => left.routeId.localeCompare(right.routeId)));
...
const ROUTES_BY_ID = new Map(PHYSICAL_GRANT_ROUTE_REGISTRY.map((route) => [route.routeId, route]));
export function physicalGrantRouteById(routeId) {
    return ROUTES_BY_ID.get(routeId) ?? null;
}
export function supportedPhysicalGrantRoute(profileId) {
...
export function physicalGrantScannerRouteDisposition(routeKey) {
...
function freezeSupportedRoute(route) {
...
function freezeUnsupportedRoute(route) {
    return Object.freeze({
        ...route,
        activationVariants: Object.freeze(route.activationVariants.map((variant) => Object.freeze(variant.map((requirement) => Object.freeze({ ...requirement }))))),
        terminalSourceUuids: Object.freeze([...route.terminalSourceUuids]),
        blocker: Object.freeze({ ...route.blocker }),
    });
}
function assertPhysicalGrantRouteRegistry(routes) {
    ...
    if (SUPPORTED_PHYSICAL_GRANT_ROUTES.length !== 5 ||
...
function assertScannerRouteDispositions(routes, dispositions) {
    ...
    for (const [routeKey, disposition] of entries) {
        if (disposition.classification !== "physical-grant")
            continue;
        if (disposition.routeIds.length === 0 ||
...
//# sourceMappingURL=physical-grant-route-registry.js.map