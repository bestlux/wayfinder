...
export function projectStructuredEidolonTraditionChoiceOptions(args) {
    if (args.sourceItemType !== "classfeature")
        return null;
    const rule = record(args.rules[args.sourceRuleIndex]);
    if (rule.key !== "ChoiceSet" || rule.flag !== "eidolonTradition")
        return null;
    if (!Array.isArray(rule.choices) || rule.choices.length !== EXPECTED_TRADITION_SKILLS.size)
        return null;
...
export function cloneStructuredEidolonTraditionValue(option) {
    return cloneData(option.installedValue);
}
function hasExactConsumerRules(rules) {
    ...
    return (traditionConsumers.length === 1 &&
...
//# sourceMappingURL=structured-eidolon-tradition-choice.js.map