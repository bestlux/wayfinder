...
interface GeneratedEntryPublication {
  title: string;
  authors: string;
  license: "OGL" | "ORC";
  remaster: boolean;
...
export function createClericPreparedEntrySource(
  actor: ActorLike,
  draft: DraftState,
  profile: "standard" | "battle-creed" = "standard"
): EmbeddedItemSource {
  return {
...
export function createBattleFontEntrySource(actor: ActorLike, draft: DraftState): EmbeddedItemSource {
  return {
    name: "Battle Font",
    type: "spellcastingEntry",
    img: WAYFINDER_ENTRY_ICON,
...
export function spellcastingEntryPublication(
  spellChoice: NonNullable<PendingStep["spellChoice"]>
): GeneratedEntryPublication | null {
...
function generatedEntryPublication(
  title: string,
  license: GeneratedEntryPublication["license"],
  remaster: boolean
): GeneratedEntryPublication {
...
export async function syncSpellcastingEntry(
  ...
  const desiredProficiency = desiredSystem.proficiency as { slug?: unknown } | undefined;
  const mergedSlots = mergeSpellcastingEntrySlots(entry?.system?.slots, desiredSystem.slots ?? {});
  const publicationManaged =
    entry.flags?.[MODULE_ID]?.generatedSpellcastingEntry === GENERATED_SPELLCASTING_ENTRY_VERSION;
...
function buildSpellcastingEntrySlots(
  ...
  if (spellChoice.destination.key.startsWith("bloodrager-")) {
    ...
    const hasRisingBloodMagic =
      Object.values(draft.selections).some((selection) => selection.uuid === RISING_BLOOD_MAGIC_UUID) ||
...
export function buildBattleCreedPreparedSlots(
  actor: ActorLike,
  draft: DraftState
): Record<string, { max: number; value: number; prepared: Array<{ id: string | null; expended: boolean }> }> {
  const currentLevel = Math.max(1, Number(actor?.system?.details?.level?.value ?? 1) || 1, draft.targetLevel || 1);
  const slots: Record<
    string,
    { max: number; value: number; prepared: Array<{ id: string | null; expended: boolean }> }
  > = {
    slot0: makePreparedSlotGroup(5),
  };
