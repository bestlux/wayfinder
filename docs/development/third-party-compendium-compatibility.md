...
| WF-COMP-004 | P1 | Fixed | Classes | `Intelligent Weapon (Alternate)` appeared for an Evil Eye character. The document says in prose that it is for the optional “Just a Weapon, Not an Ancestry” rules and that PF2E's boost manager is incompatible. | Ordinary class roots must now expose the structured key-ability, HP, perception, saves, attacks, defenses, class-DC, and skill progression needed for a PC. The alternate pseudo-class is hidden; broader partial-support disclosure remains tracked by WF-COMP-010. |
