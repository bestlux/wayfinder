...
export function withClassArchetypeInitialTraining(
  document: unknown,
  profile: ClassArchetypeProfile | null | undefined
): unknown {
  if (!isRecord(document) || !profile?.initialTraining || extractDocumentSlug(document) !== profile.classSlug) {
    return document;
  }

  const source: unknown = typeof document.toObject === "function" ? document.toObject() : document;
  if (!isRecord(source)) return document;
 ...
 */
export function classArchetypeInitialTrainingProjection(
  document: unknown,
  profile: ClassArchetypeProfile | null | undefined
): unknown {
  const projected = withClassArchetypeInitialTraining(document, profile);
  if (projected === document || !profile || !isRecord(projected) || !isRecord(projected.system)) return projected;
  const rules = projected.system.rules;
  if (!Array.isArray(rules)) return projected;
  return {
    ...projected,
    system: {
      ...projected.system,
