...
export async function prepareAdventurersPackExpansion(args) {
    ...
    const granted = await Reflect.apply(createGrantedItems, args.kitDocument, [{ size: pf2eSize }]);
    if (!Array.isArray(granted))
        throw new Error("PF2E returned malformed Adventurer's Pack granted items.");
    ...
    for (const profile of PROFILE) {
        ...
        if (!itemType || !name)
            throw new Error(`Adventurer's Pack child ${profile.sourceUuid} is not a physical item.`);
        sources.set(profile.expansionPath, cloneData(source));
        items.push({
            ...profile,
...
export function isQualifiedKitSource(sourceUuid) {
    return sourceUuid === ADVENTURERS_PACK_UUID;
}
function item(expansionPath, parentPath, sourceUuid, quantity) {
    return Object.freeze({ expansionPath, parentPath, sourceUuid, quantity });
}
function assertStableKitPaths(rawItems) {
...
function grantedItemFacts(value) {
...
function assertGrantedProfile(granted, expectedSize) {
    if (granted.length !== PROFILE.length)
        throw new Error("PF2E Adventurer's Pack expansion cardinality changed.");
    const byUuid = new Map(granted.map((entry) => [entry.sourceUuid, entry]));
    if (byUuid.size !== PROFILE.length)
        throw new Error("PF2E Adventurer's Pack expansion contains duplicate sources.");
    for (const profile of PROFILE) {
...
function materializedSize(size) {
    return size === "tiny"
        ? "tiny"
...
function documentSource(document) {
    ...
    const source = typeof toObject === "function" ? Reflect.apply(toObject, document, [true]) : document;
    return isRecord(source) ? cloneData(source) : null;
}
function sourceUuidOf(source) {
...
//# sourceMappingURL=pf2e-kit-adapter.js.map