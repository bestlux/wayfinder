...
export async function createEmbeddedSource(
  ...
  if (selection.itemType === "class" && draft) {
    source = withClassArchetypeInitialTraining(
      source,
      resolveActiveClassArchetypeProfile(draft, steps, [])
    ) as EmbeddedItemSource;
...
async function applyPendingStaticGrantPreselectChoices(
  ...
  for (const [ruleIndex, rule] of rules.entries()) {
    ...
    const existingPreselectChoices = isLooseRecord(rule.preselectChoices)
      ? Object.fromEntries(
          Object.entries(rule.preselectChoices).filter(
            (entry): entry is [string, string] => typeof entry[1] === "string"
          )
        )
      : {};
    const resolvedPreselectChoices: Record<string, string> = {
      ...preselectChoices,
...
function resolveGrantItemPreselectChoiceReferences(source: EmbeddedItemSource): void {
  const rules = Array.isArray(source.system?.rules) ? source.system.rules : [];
  if (rules.length === 0) {
    return;
  }
...
function resolveRuleSelectionReference(source: EmbeddedItemSource, value: string): string | null {
