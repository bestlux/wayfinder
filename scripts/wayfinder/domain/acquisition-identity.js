...
export async function prepareAcquisitionIdentityPlan(args) {
    ...
    if (!draft)
        throw new TypeError("Acquisition identity preparation requires canonical draft state.");
    ...
    if (draft.policySnapshot.material.subject.actorId !== actorId ||
...
export function isPreparedAcquisitionIdentityPlan(value) {
...
export function assertPreparedAcquisitionIdentityPlanMatches(args) {
    ...
    if (plan.subject.actorId !== actorId ||
    ...
    if (!current)
        throw new TypeError("The current acquisition draft is malformed.");
    const resolvedAllowances = new Map();
    for (const entry of plan.entries) {
        if (!entry.resolvedAllowanceId)
            continue;
        for (const lineId of entry.lineIds)
            resolvedAllowances.set(lineId, entry.resolvedAllowanceId);
    }
...
function captureCurrentMaterialFacts(draft, resolvedAllowances) {
...
function combinedPrice(price, requestedQuantity, materializedQuantity) {
...
function prefixedOpaqueId(prefix, value) {
...
function safeAdd(left, right) {
    ...
    if (!Number.isSafeInteger(result))
        throw new RangeError("Acquisition quantity exceeds safe integer arithmetic.");
    return result;
}
...
//# sourceMappingURL=acquisition-identity.js.map