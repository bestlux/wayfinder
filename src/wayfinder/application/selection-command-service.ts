...
interface ChooseSelectionOptionDependencies {
  ...
  resolveSelectionClassHasSpellcasting: (selection: SelectionRef | null) => Promise<boolean>;
  ...
  invalidateCampaignFeatSelectionsByFeatType: (featType: string) => Promise<string[]>;
...
export async function chooseSelectionOption(
  ...
  if (step.slotKind === "class" && previousSelection?.uuid !== selection.uuid) {
    ...
    if (previousClassSlug !== nextClassSlug || spellcastingCapabilityChanged) {
      ...
      if (
        ...
        const featScope =
          ancestryFeatInvalidated.length > 0 || campaignAncestryFeatInvalidated.length > 0
  ...
  if (
    (step.slotKind === "ancestry-feat" || (step.slotKind === "campaign-feat" && selection.featType === "ancestry")) &&
