...
export function buildBloodragerSpellChoiceSteps(params) {
    if (params.targetLevel < 2 ||
        !params.effectiveClassFeatureDocuments.some((document) => classArchetypeProfileForDocument(document)?.value === "bloodrager")) {
        return [];
    }
    ...
    const assignedSpellUuids = new Set();
...
function bloodragerTradition(params) {
    ...
    for (const document of params.effectiveClassFeatureDocuments) {
        if (document.system?.slug !== "bloodrager-dedication" &&
            ![document.sourceId, document.flags?.core?.sourceId, document._stats?.compendiumSource].includes(BLOODRAGER_DEDICATION_UUID)) {
            continue;
        }
        const flags = document.flags;
        const selections = flags?.pf2e?.rulesSelections;
        if (selections && Object.hasOwn(selections, "skill"))
            return traditionForSkill(selections.skill);
        const systemSelections = flags?.system?.rulesSelections;
        if (systemSelections && Object.hasOwn(systemSelections, "skill"))
            return traditionForSkill(systemSelections.skill);
    }
    return null;
}
function traditionForSkill(skill) {
    return skill === "arcana" ? "arcane" : skill === "religion" ? "divine" : null;
}
//# sourceMappingURL=bloodrager-step-builder.js.map