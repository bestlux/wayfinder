...
export async function applyClassFeatureChoiceDraft(actor, draft, steps, deps) {
    ...
    const staticGrantPreparations = await Promise.all(groups
...
async function prepareStaticGrantOwner(actor, draft, steps, deps, group) {
    ...
    const matchingGrants = readManualStaticItemGrants(ownerSource).filter((grant) => grant.uuid === group.sourceSelection.uuid);
    ...
    const featureSource = ownerItem && hasFeatureMutations(group)
...
function assertFeatureGroupRuleAuthority(actor, group, featureSource, allowMissing) {
    if (!hasFeatureMutations(group))
        return;
    if (!featureSource) {
        throw new Error(`Cannot reconcile ${group.sourceSelection.name}: its prepared child source is unavailable.`);
    }
    const matches = listActorItems(actor).filter((item) => itemMatchesSourceId(item, group.sourceSelection.uuid));
    ...
    for (const entry of group.grantEntries) {
        ...
        if (!sameChoiceRuleIdentity(actorChoiceRule, preparedChoiceRule) ||
...
function sameChoiceRuleIdentity(actorRule, preparedRule) {
    if (!isRecord(actorRule) || !isRecord(preparedRule))
        return false;
    if (actorRule.key !== "ChoiceSet" || preparedRule.key !== "ChoiceSet")
        return false;
    return actorRule.flag === preparedRule.flag && actorRule.slug === preparedRule.slug;
}
function sameGrantRuleIdentity(actorRule, preparedRule) {
    if (!isRecord(actorRule) || !isRecord(preparedRule))
        return false;
    return (actorRule.key === "GrantItem" &&
...
async function convergeExistingStaticGrantOwner(actor, deps, preparation) {
    if (!preparation.ownerItem) {
        return;
    }
    const currentOwnerItems = listActorItems(actor).filter((item) => itemMatchesSourceId(item, preparation.owner.selection.uuid));
    if (currentOwnerItems.length !== 1 ||
        !currentOwnerItems[0]?.id ||
        currentOwnerItems[0].id !== preparation.ownerItem.id) {
        throw new Error(`Cannot reconcile ${preparation.group.sourceSelection.name}: its planned static grant owner changed during Apply.`);
    }
    ...
    try {
        ...
        try {
            ...
            throw new AggregateError([error, rollbackError], `Failed to reconcile ${preparation.owner.selection.name} safely.`, {
                cause: rollbackError,
            });
        }
        throw error;
...
function assertStaticGrantOwnerAuthority(item, owner) {
...
function canonicalFeatLocation(slotId) {
...
function collectFeatureGroups(draft, steps) {
    ...
    for (const step of steps) {
        if (step.kind === "pick-item" && step.grantSelection) {
            ...
            if (existingGroup) {
                group.staticGrantOwner = reconcileStaticGrantOwner(group.staticGrantOwner, step.grantSelection.staticGrantOwner ?? null, group.sourceSelection.name);
            }
        ...
        if (step.kind === "class-choice" && step.classChoice) {
            ...
            if (existingGroup) {
                group.staticGrantOwner = reconcileStaticGrantOwner(group.staticGrantOwner, step.classChoice.staticGrantOwner ?? null, group.sourceSelection.name);
            }
...
function hasFeatureMutations(group) {
    return group.choiceEntries.length > 0 || group.grantEntries.length > 0;
}
function reconcileStaticGrantOwner(current, candidate, sourceName) {
