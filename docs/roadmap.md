...
### 0.8.5 — Wizard curriculum compatibility

Wizard curriculum parsing now follows PF2E's ordered description sections across direct list rows, paragraph-wrapped list rows, and standalone curriculum paragraphs. This restores Quantic Control, Keen Inquiry, Mentalism, Nexian Spaces, and Magical Technologies while retaining existing schools and selected Additional Curriculum and Sin Spells contributors. Supplemental merging is restricted to wizard-trait class features so Oracle and Animist spell lists cannot leak into multiclass wizard curriculum choices. See the [targeted patch record](coverage/beta-readiness-smoke.md#2026-08-24-085-targeted-patch-qualification).

### 0.8.6 — Actor-owned deity reconciliation

An unresolved Cleric deity selector now keeps one eligible actor-owned deity visible and adopts that loose item into the selector's bidirectional PF2E grant graph during Apply. Adoption is an explicit deity-only capability: ambiguous matches, existing foreign provenance, malformed item types, feats, spells, native grants, and other selector routes retain their fail-closed behavior. See the [targeted patch record](coverage/beta-readiness-smoke.md#2026-08-26-086-targeted-patch-qualification).
