...
The historical baseline artifacts are `.wayfinder-smoke/release-0.5.0-baseline-final` and `.wayfinder-smoke/release-0.5.0-free-archetype-final`. The last published full-release evidence is recorded under the six `.wayfinder-smoke/release-0.7.3-rc-*` artifacts listed in the release smoke log; the 0.8.0 exact-candidate matrix is not yet recorded as complete.
