...
export function startingEquipmentResultWindowForViewport(input: {
  ...
  const firstVisibleRow = startingEquipmentIndexAtScrollOffset(
    scrollTop,
    input.total,
    measurements.measuredRows ?? new Map(),
    estimatedRowPx
  );
  const chunk = STARTING_EQUIPMENT_RESULT_WINDOW.hydrationChunkSize;
  const offset =
    Math.floor(Math.max(0, firstVisibleRow - STARTING_EQUIPMENT_RESULT_WINDOW.overscanRows) / chunk) * chunk;
...
export function clampStartingEquipmentRowHeight(value: number): number {
  ...
  return Math.min(
    STARTING_EQUIPMENT_RESULT_WINDOW.maximumRowHeightPx,
    Math.max(STARTING_EQUIPMENT_RESULT_WINDOW.minimumRowHeightPx, value)
  );
}
