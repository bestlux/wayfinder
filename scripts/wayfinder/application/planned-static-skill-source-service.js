...
export function activePlannedClassArchetypeProfile(draft, steps) {
    ...
    if (!steps.some((step) => step.kind === "class-archetype")) {
        for (const [slotId, value] of Object.entries(draft.classArchetypeChoices)) {
            const profile = classArchetypeProfile(value);
            if (profile?.decisionSlotId === slotId &&
 ...
 */
export function synchronizeRetainedClassArchetypeChoice(draft, steps, actorDocuments) {
    retainActiveClassArchetypeChoices(draft, steps);
    if (activePlannedClassArchetypeProfile(draft, steps))
        return;
    if (steps.some((step) => step.kind === "class-archetype" && typeof draft.classArchetypeChoices[step.slotId] === "string")) {
        return;
    }
...
function retainedClassArchetypeProfile(draft, steps, actorDocuments) {
    const resolution = inspectRetainedClassArchetypeProfileDocuments(actorDocuments);
...
function activePlanReferencesProjectedProfileGrant(profile, targetLevel, steps) {
    ...
    return (activeProfileSourceUuids.size > 0 &&
        steps.some((step) => stepSourceUuids(step).some((uuid) => activeProfileSourceUuids.has(uuid))));
}
function stepSourceUuids(step) {
    if (step.kind === "skill-training") {
        return [...step.training.choiceRules, ...step.training.loreChoices].flatMap((choice) => choice.persistence ? [choice.persistence.sourceUuid] : []);
    }
    if (step.kind === "singleton-choice")
        return [step.singletonChoice.sourceUuid];
    if (step.kind === "class-choice")
        return [step.classChoice.sourceUuid];
    if (step.kind === "spell-choice")
        return step.spellChoice.sourceUuid ? [step.spellChoice.sourceUuid] : [];
