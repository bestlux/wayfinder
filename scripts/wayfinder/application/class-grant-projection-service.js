...
export async function projectCurrentClassGrants(actor, draft, activeSteps, options = {}) {
    ...
    const currentSnapshot = createAcquisitionPolicySnapshot(currentPolicy, acquisition.recipe, acquisition.recipeSelection);
...
export function captureObservedClassGrantItems(actor) {
    if (!isRecord(actor) || !nonEmpty(actor.id))
        throw new TypeError("Class-grant observation requires an actor ID.");
    ...
    const visit = (item) => {
        ...
        const rawIdentity = isRecord(item.flags) && isRecord(item.flags[MODULE_ID]) ? item.flags[MODULE_ID].acquisition : null;
        ...
        const grantedByItemId = isRecord(item.flags) &&
...
function observedClassGrantQuantity(item) {
    const isOfType = item.isOfType;
    const physical = typeof isOfType === "function"
...
export async function projectPlannedClassGrants(args) {
    ...
    const activeProfile = classSelection.uuid === UUIDS.alchemistClass ||
        (classSelection.uuid === UUIDS.investigatorClass &&
            Object.values(args.draft.branchSelections).some((selection) => selection.slotId === "class-branch-methodology-level-1" &&
                selection.uuid === UUIDS.alchemicalSciences &&
                originIsActive(selection, args.activeSteps, args.observedActorItems))) ||
        (classSelection.uuid === UUIDS.barbarianClass &&
            Object.values(args.draft.branchSelections).some((selection) => selection.slotId === "class-branch-instinct-level-1" &&
                selection.uuid === UUIDS.giantInstinct &&
                originIsActive(selection, args.activeSteps, args.observedActorItems)));
    if (!activeProfile)
        return completeProjection(args, grants, blockers);
    ...
    const alchemicalSciences = branchSelections.find((selection) => selection.slotId === "class-branch-methodology-level-1" &&
        selection.uuid === UUIDS.alchemicalSciences &&
        originIsActive(selection, args.activeSteps, args.observedActorItems));
    ...
    const giantInstinct = branchSelections.find((selection) => selection.slotId === "class-branch-instinct-level-1" &&
        selection.uuid === UUIDS.giantInstinct &&
        originIsActive(selection, args.activeSteps, args.observedActorItems));
    if (classSelection.uuid === UUIDS.barbarianClass && giantInstinct) {
        const result = await projectTitanMauler(classDocument, giantInstinct, args.draft.acquisition?.lines ?? [], args.currentEquipmentPolicy ?? null, args.actorSize ?? null, args.resolveCharacterAccessRef, args.fetchDocumentByUuid, args);
        if (result.grant)
            grants.push(result.grant);
        if (result.blocker)
            blockers.push(result.blocker);
...
async function projectAlchemist(classDocument, fetchDocumentByUuid) {
    ...
    if (!alchemy || !formulaBook || !formulaBookItem)
        return sourceMissing(profileId);
    if (!hasExactlyOneStaticGrant(alchemy, UUIDS.formulaBookFeature) ||
...
async function projectInvestigator(classDocument, selection, fetchDocumentByUuid) {
    ...
    const formulaBookItem = await fetchDocumentByUuid(UUIDS.formulaBookItem);
    if (!methodology || !alchemicalSciences || !formulaBookItem)
        return sourceMissing(profileId);
    if (!hasDynamicBranch(methodology, "methodology", "investigator-methodology") ||
...
async function projectTitanMauler(classDocument, selection, acquisitionLines, policy, actorSize, resolveCharacterAccessRef, fetchDocumentByUuid, subject) {
    ...
    if (!instinct || !giant)
        return sourceMissing(profileId);
    if (!hasDynamicBranch(instinct, "instinct", "barbarian-instinct") ||
...
function createRegisteredClassGrant(profileId, overrides = {}) {
    ...
    const eligibilityEvidence = overrides.eligibilityEvidence ??
        (route.classification === "supported-native" ? { kind: "fixed-native-profile" } : null);
...
function completeProjection(subject, grants, blockers) {
    return {
        grants,
        blockers,
        preparedPlan: blockers.length === 0
...
function profileIdForSelection(classUuid, draft) {
    if (classUuid === UUIDS.alchemistClass)
        return "alchemist-formula-book";
    if (classUuid === UUIDS.investigatorClass &&
...
function sourceMissing(profileId) {
    return {
        blocker: {
            code: "source-missing",
            profileId,
...
function hasDynamicBranch(document, flag, otherTag) {
    ...
    const grants = values.filter((rule) => rule.key === "GrantItem" && rule.uuid === `{item|flags.system.rulesSelections.${flag}}`);
    const choice = choices[0];
    return (choices.length === 1 &&
        !!choice &&
...
function isReviewedFormulaBookItem(document) {
...
function selectedOfficialRecipe(recipe) {
...
function hasTitanMaulerSemanticCanaries(document) {
    if (!isRecord(document) || !isRecord(document.system) || !isRecord(document.system.description))
        return false;
    ...
    const normalized = text
        ...
        .replace(/\s+/g, " ")
        .toLowerCase();
    return (normalized.includes("price of 9 gp or less") &&
...
export function buildTitanMaulerCandidate(args) {
    ...
    if (sourceUuid !== null && sourceUuid !== line.sourceUuid)
        return null;
    const packMatch = /^Compendium\.([^.]+\.[^.]+)\.Item\.[^.]+$/u.exec(line.sourceUuid);
    if (!packMatch || line.policyDecision.packId !== packMatch[1])
        return null;
    ...
    if (basePriceCopper === null || lineBasePriceCopper !== basePriceCopper)
        return null;
    ...
    if (!Number.isSafeInteger(materializedQuantity))
        return null;
    return {
        sourceUuid: line.sourceUuid,
        itemType: "weapon",
...
function copperValue(value) {
    const denominations = new Set(["pp", "gp", "sp", "cp"]);
    if (Object.keys(value).some((key) => !denominations.has(key)))
        return null;
    let total = 0;
...
function hasOtherTag(document, tag) {
...
//# sourceMappingURL=class-grant-projection-service.js.map