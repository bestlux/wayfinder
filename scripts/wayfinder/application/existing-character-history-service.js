...
export function buildExistingCharacterHistory(actor, now = () => new Date().toISOString()) {
    ...
    for (const level of ABILITY_BOOST_LEVELS.filter((candidate) => candidate <= actorLevel)) {
        ...
        entries.push(levelBoosts.length === 4
            ? mappedEntry(`ability-boosts-level-${level}`, level, "ability-boost", level === 1 ? "Level 1 free ability boosts" : `Level ${level} ability boosts`, levelBoosts.map((ability) => ability.toUpperCase()).join(", "), null)
            : reviewEntry(`ability-boosts-level-${level}`, level, "ability-boost", level === 1 ? "Level 1 free ability boosts" : `Level ${level} ability boosts`, "Review required: no complete level-specific boost record is stored on the actor"));
    }
