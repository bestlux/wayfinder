import { getDocumentRules, isChoicePredicate, isRecord, matchesChoicePredicateListAgainstRollOptions, toNonEmptyString, } from "../rule-data.js";
...
export function projectRegisteredDynamicChoices(documents) {
    ...
    for (const document of documents) {
        for (const rule of getDocumentRules(document)) {
            ...
            const choices = choicesByPath.get(path) ?? new Map();
            choices.set(choice.value, choice);
            choicesByPath.set(path, choices);
        }
    }
    return Object.fromEntries(Array.from(choicesByPath, ([path, choices]) => [
        path,
...
export function resolveRegisteredDynamicChoices(args) {
    ...
    return (args.projectedChoices?.[args.path] ?? []).filter((choice) => matchesChoicePredicateListAgainstRollOptions(choice.predicate, activeRollOptions));
}
function projectedChoice(value) {
    if (!isRecord(value)) {
        return null;
    }
...
function sourceCompatibilityRollOptions(sourceDocument) {
    const otherTags = sourceDocument?.system?.traits
        ?.otherTags;
    return new Set((Array.isArray(otherTags) ? otherTags : [])
        .filter((tag) => typeof tag === "string")
        .map((tag) => `parent:tag:${tag.trim().toLowerCase()}`)
        .filter((tag) => tag !== "parent:tag:"));
}
//# sourceMappingURL=dynamic-choice-registry.js.map