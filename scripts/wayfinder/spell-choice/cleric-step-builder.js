...
export function buildClericSpellChoiceSteps(params) {
    const { draft, effectiveClassDocument, effectiveDeityDocument, effectiveClassFeatureDocuments, readExistingSpellChoiceSelections, classSlug, } = params;
    ...
    const usesBattleCreed = isBattleCreedSelected(draft) || effectiveClassFeatureDocuments.some((document) => documentIsBattleCreed(document));
    const steps = [];
