...
The exact candidate `4d7d2c38cd444b05d54adc10804730fdce26e79f` passed the full coordinator in `.wayfinder-smoke/wf51-0.8.10-issue37-final`: all 11 children, 55 matrix executions / 54 unique scenarios, both locales, recovery, and all 15 release-overlay rows. An earlier coordinator run passed its character matrix but stopped before acquisition because the non-GM account environment variable was omitted; the final run supplied the existing test account and reran the complete qualification without code changes.

Tag `v0.8.10` and release workflow `34300940943` shipped that exact candidate. All three workflow jobs passed, including Foundry dry-run and registration. The 280-entry public ZIP matches the qualified ZIP and GitHub asset digest at SHA-256 `b6d0b3d2fc449e2fcaee8ed1723b5090b21b98e1b4e61e1551dadcb6ad852a19`. Unauthenticated downloads verified identical versioned/latest manifests, matching archive entries, and the embedded version-specific download URL. The public Foundry package page lists 0.8.10. Local and remote master, tag, workflow head, and qualification candidate agreed at publication; master advances only with this evidence record.
...
The focused Minotaur, Aiuvarin, Cathedral Child, Spirit Barbarian case passed on Wayfinder 0.8.9 / Foundry 14.367 / PF2E 8.5.0 in `testing-world`. Artifact `.wayfinder-smoke/pf850-hotfix-0.8.9-exact` applied all ten planned steps, completed retain-all with 15 gp intact, opened no native choice dialogs, cleaned up its fixture, and returned zero pending steps on rerun.

The first full coordinator attempt exposed a smoke-probe collision: the review announcement selector read the new compatibility banner instead of the equipment status message. The probe now targets the equipment status panel. Fresh artifact `.wayfinder-smoke/wf51-release-0.8.9-3b4e67b` passed all eleven children: 55 character-build executions / 54 unique scenarios, ten acquisition cases, five Wave 3 cases, three Wave 4 cases, English and Chinese keyboard/layout coverage, seven focused cases, and the ordered fifteen-row release overlay. Both locales recorded real review and failure announcement changes. The compatibility banner was also visually checked at 760 px and 1240 px, with Apply available for a completed build. Cleanup removed the guarded fixtures and restored actor counts, policy, judgments, variant settings, packs, and language.

Completion evidence `fd624432-52f6-45a1-b3e1-495e731fec7e` binds run `5ef0deae-c256-4c90-a2c3-0258d7043e0b` to candidate `3b4e67bfbf6dea3c619c775ec057177b9cf1222f`, served-script manifest SHA-256 `4a2b344ae99adc9ab34ca5a0efb8b13309b31d82bf0ee54c80a06c39495d38a2`, and result SHA-256 `42ad86109a3380d71540740fc9da6821311d4681e270c7a71e067d0aea18845b`. Every child recorded no candidate drift. Qualified packaging in `dist/qualified-release-0.8.9` bound that evidence to a 280-entry ZIP with SHA-256 `ab5293ee4c5433cedec5ce062f7dae15edb32694789125f2114b56b9ae9385e0`. The release retains PF2E 8.4.1 as its minimum and advertises the exact tested 8.5.0 runtime as verified; this does not claim a fresh 8.4.1 runtime rerun.

Candidate `3b4e67bfbf6dea3c619c775ec057177b9cf1222f` was published as `v0.8.9` by [workflow 34053548416](https://github.com/bestlux/wayfinder/actions/runs/34053548416). Validation, GitHub publication, and Foundry registration all succeeded, including Foundry API dry-run and actual publication responses. A fresh unauthenticated fetch of the [public package page](https://foundryvtt.com/packages/wayfinder-pf2e) listed 0.8.9 verified for Foundry 14.367. Independent downloads proved the versioned and `latest` manifests byte-identical, the public ZIP identical to the qualified package and GitHub asset digest, and all 280 entries consistent with the package manifest. The embedded manifest identifies the same version and version-pinned download. Local and remote `master`, tag `v0.8.9`, workflow head, and candidate agreed at publication; `master` advances only with this post-release evidence record. Neither release manifest contains marketplace media URLs.
...
Because this patch changes a shared Starting Equipment command boundary, the exact clean candidate repeated the full WF-080-51 Starting Equipment coordinator. Artifact `.wayfinder-smoke/wf51-release-0.8.8-0bc0820` passed 55 executions / 54 unique scenarios, the ten-case acquisition tracer, five Wave 3 cases, three Wave 4 cases, English and Chinese experience coverage, all seven focused cases, and the ordered fifteen-row release overlay. Completion evidence `d54d36db-e7b7-46c8-b237-c4218cd51b39` binds run `71b09ae1-e141-43ac-b31c-5aadf0df75fd` to candidate `0bc0820f7d5abcec07eb3c815fb1b5f48d21fb4d`, served-script manifest SHA-256 `732ac7224aff7e0cd593c269c84081fcfb26dfc4ca06ad469b9ee207467b5308`, and result SHA-256 `9ddee986dcafc51c9dfc69c3ee390ecaa1b9753826e9d96edbeccbc87dd9430f`. Candidate drift was false for every child, and final recovery removed the guarded fixtures and restored exact actor counts, policy, judgments, PF2E variants, packs, and language. The permanent smoke matrix now includes the reported build as `barbarian-minotaur-aiuvarin-cathedral-child-l1-apply-rerun`; it remains a separate focused run because WF-080-51's fixed baseline does not automatically add newly registered cases.

Qualified packaging bound that evidence to a 280-entry ZIP with SHA-256 `808b674b86006d233c15e5eb04c4c720f505dac5f9985c6f1b7c96d645000b97`. The source, release, and embedded manifests all identify 0.8.8, use version-pinned download and changelog URLs, and advertise Foundry 14.367 / PF2E 8.4.1 compatibility.

Release candidate `0bc0820f7d5abcec07eb3c815fb1b5f48d21fb4d` was published as `v0.8.8` by [workflow 33281961841](https://github.com/bestlux/wayfinder/actions/runs/33281961841). Validation, GitHub publication, and Foundry package registration all completed successfully; the Foundry job recorded both dry-run and actual publish success, and the [public package page](https://foundryvtt.com/packages/wayfinder-pf2e) lists version 0.8.8 verified for Foundry 14.367. Fresh unauthenticated downloads proved the public versioned and `latest` manifests byte-identical and the public ZIP identical to the qualified package hash above. The ZIP contains the same 280 installable entries as its public package manifest. Local and remote `master`, tag `v0.8.8`, workflow head, and release candidate agreed exactly at publication; `master` advances only with this post-release evidence record. Neither release manifest contains marketplace media URLs.
...
Release candidate `3c90c36ab3921a19a7acc97c0746e143c214723d` was published as `v0.8.7` by [workflow 33120231533](https://github.com/bestlux/wayfinder/actions/runs/33120231533). Validation, GitHub publication, and Foundry package registration all completed successfully; the Foundry job recorded both the dry-run and actual publish success responses, and the [public package page](https://foundryvtt.com/packages/wayfinder-pf2e) lists version 0.8.7 verified for Foundry 14.367. The public versioned and `latest` manifests both identify 0.8.7 and resolve to the versioned ZIP. The unauthenticated public ZIP contains the same 279 installable entries as its package manifest and has SHA-256 `a9fbab1ed7716ef292cb473c6241ca5b5acc31624b731c2e8186ff7b3a6c75de`, matching the local package, workflow package manifest, and GitHub asset digest. The local tag, remote tag, workflow head, and release candidate commit agree exactly; `master` advances only with this post-release evidence record. Neither release manifest contains marketplace media URLs.
...
Release candidate `254fa6c691f19cc96869364fda99f20d6bed67fb` was published as `v0.8.6` by [workflow 33020497723](https://github.com/bestlux/wayfinder/actions/runs/33020497723). Validation, GitHub publication, and Foundry package registration all completed successfully; the Foundry job recorded both the dry-run and actual publish success responses, and the [public package page](https://foundryvtt.com/packages/wayfinder-pf2e) lists version 0.8.6 verified for Foundry 14.367. The public versioned and `latest` manifests both identify 0.8.6 and resolve to the versioned ZIP. The unauthenticated public ZIP contains the same 278 installable entries as its package manifest and has SHA-256 `8370b6be9a0d7520f0a1bb76b88c9da49aa29d87a0bdd6e2f8a0191bbd6cbcab`, matching the local package, workflow package manifest, and GitHub asset digest. The local tag, remote tag, and workflow head all resolve to the exact candidate commit; `master` advances only with this post-release evidence record. Neither release manifest contains marketplace media URLs.
...
Release candidate `df4c92490bbcbfeeec48043be3067721df525ef9` was published as `v0.8.5` by [workflow 32770223787](https://github.com/bestlux/wayfinder/actions/runs/32770223787). Validation, GitHub publication, and Foundry package registration all completed successfully. The public versioned and `latest` manifests both identify 0.8.5 and resolve to the versioned ZIP; the unauthenticated public ZIP SHA-256 is `3d6fff9d1d06aba707ec4587ebd8d2beb0d585ca7190acc81148350b58e95498`, matching the local package, workflow package manifest, and GitHub asset digest. The release tag and workflow head both resolve to the exact candidate commit; `master` advances only with this post-release evidence record. The release manifest contains no marketplace media URLs.
