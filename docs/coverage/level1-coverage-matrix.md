...
Release 0.9.0 adds Bloodrager and Vindicator to the three previously registered class-archetype profiles, with scope through level 5 and Free Archetype disabled. Their Free Archetype combinations require manual GM setup and are not qualified. Qualification and publication evidence is recorded in the [live smoke log](./beta-readiness-smoke.md). The historical audit evidence below remains separately dated.
...
- Prior full-regression evidence checked on 2026-08-14: Wayfinder 0.7.3 passed 55 executions representing 54 unique scenarios against Foundry VTT 14.366 and PF2E 8.4.0, with zero classified/manual and zero failed. The matrix includes all-class baselines, seven unique existing-character upgrades, three Free Archetype paths, Ancestry Paragon, Gradual Ability Boosts, five profiles through level 10, Fighter through level 20, and a deliberate late-phase Apply failure/retry. See [the smoke log](beta-readiness-smoke.md).
- Release 0.8.0 evidence covers Starting Equipment acquisition, all forced retry boundaries, supported physical grants, actor-size and stack pricing, Adventurer's Pack containment, source isolation, English/Chinese keyboard flows, qualifying full catalogue profiles, and the exact-candidate 55-execution regression against Foundry VTT 14.366 / PF2E 8.4.1. Package binding, independent review, and publication completed on 2026-08-22; see [the smoke log](beta-readiness-smoke.md).
- Historical full-release evidence for 0.8.3 against Foundry VTT 14.366 / PF2E 8.4.1 records the passing 55-execution / 54-scenario matrix and every Starting Equipment overlay, including a focused live proof that a level-1 character can find and prepare Healing Potion (Minor). Exact-candidate qualification, package verification, publication, and Foundry registration completed on 2026-08-23; see [the dated smoke entry](beta-readiness-smoke.md#2026-08-23-release-083).
- Historical class-archetype evidence covers Standard and registered-profile paths for Cleric/Battle Creed, Gunslinger/Way of the Spellshot, and Investigator/Palatine Detective. Those three profiles passed direct and incremental level-1-to-5 apply/rerun; Battle Creed also retains both-skills-trained, actor-owned Toughness, and same-draft Shielded Fortune fallback cases. Bloodrager/Vindicator have separate supplemental release scenarios; see the current live smoke log for their outcomes.
...
| Class archetype selection | Guided through level 5 for five registered profiles | Dedicated class-archetype decision and profile contributor | Battle Creed, Way of the Spellshot, Palatine Detective, Bloodrager, and Vindicator are registered. Bloodrager replaces initial training with Athletics/Medicine and 2 + Intelligence modifier additional skills; Vindicator replaces Nature with Religion, guides deity/sanctification, and sets up divine Wisdom-based Vindicator's Mark and focus. Each affected class retains an explicit Standard choice; the other eight profiles remain filtered. |
| Class branches | Guided when PF2E rule data is structured | Branch-discovery and branch-selection steps | Works when the class exposes selector-style branch rules cleanly. Registered class archetypes use the dedicated lane instead; unregistered class-archetype options stay filtered. Options in predicate-backed branch steps stay visible as before; options in tag-based branch steps are filtered unless every embedded `ChoiceSet` is covered by an existing guided lane. |
| Class-owned granted selections | Guided when PF2E rule data is structured | Granted-item and class-choice steps | Includes flows like deity-linked or class-linked granted selections when discoverable from rules. Same-item class-choice option predicates are supported when later choices depend on roll options from earlier choices on the same class feature. |
| Class-owned spell choices | Guided when PF2E rule data is structured | Shared caster profiles plus class contributor seam | Current live matrix covers prepared, spontaneous, bounded, spellbook, and innate families, including Spellshot and Palatine Detective profile destinations. Daily preparation and untested branch variants remain PF2E-native/manual. |
...
| Standalone filtered feat choices | Guided when PF2E rule data is structured | Flag-choice workflow | Covers filtered `ChoiceSet` rules without a consuming `GrantItem` when the selected flag feeds PF2E-native rules. Actor-placeholder predicates require known draft context, and selected-item or equipment item types remain unsupported. |
...
| Starting Equipment | Guided for eligible new or replacement characters | Dedicated catalogue, cart, review, prepared acquisition, and durable receipt | Level 1 uses the official 15 gp budget. GM policy controls sources and exceptions; supported prepared-build grants are no-charge. Foreign or unresolved wealth hands off with zero writes. Adventurer's Pack expansion is supported; Quick Equipment Packages are not included. |
...
| Default and upgrade paths | Registered class archetypes | Standard/profile Cleric, Gunslinger, Investigator, Barbarian, and Ranger; direct and incremental profile actors; profile fallback fixtures | Choose a registered profile and complete target level 5 | Replaced selector stays hidden, level-2 dedication occupies the reserved class feat, level-4 class feat remains, profile training/spells/grants apply to the correct destination, and rerun has zero pending steps | Historical `.wayfinder-smoke/release-0.4.0-full-4`; supplemental expansion cases and current `beta-readiness-smoke.md`; class-archetype tests | release |
| Upgrade path | Free Archetype | PF2E variant enabled; blank or existing Fighter/Archer and Rogue/Acrobat actors | Complete target level 5 and rerun | Separate class and archetype slots render at levels 2 and 4; archetype choices occupy `archetype-2`/`archetype-4`; rerun has zero pending steps | `.wayfinder-smoke/release-0.5.0-free-archetype-final`, focused tests | release |
...
| Failure mode | Unsupported structured rule shape | Content exposes unsupported selected-item, equipment, dynamic flags-path, cross-item, or multi-ChoiceSet graph dependencies | Build plan | Wayfinder avoids guessing; unsupported choices remain PF2E-native/manual | AP/side-book audit gap table | release |
| Failure mode | Direct feat or branch embeds an unsupported `ChoiceSet` | A selected feat or class-branch option needs an internal choice Wayfinder cannot yet render | Build picker options | Direct feat options and tag-based branch options are hidden unless every embedded `ChoiceSet` is covered by a guided follow-up lane; predicate-backed branch steps keep their curated options visible | `tests/pack-options.test.ts`, live smoke harness | release |
...
- Class archetypes now have a dedicated three-state decision (`unanswered`, `standard`, or a registered profile) and a registry seam for cross-cutting planning and apply behavior.
...
- Direct feat options and tag-based class-branch options that embed their own `ChoiceSet` rules now surface only when every embedded choice is claimed by a guided lane: grant choice, flag choice, singleton choice, skill training, or supported class-choice discovery. Predicate-backed branch steps keep their curated options visible, since those flows were already guided end-to-end (for example Psychic conscious minds). Unsupported selected-item, equipment, dynamic flags-path, and cross-item dependency shapes remain hidden from guided pickers for now.
...
- Predicate-gated singleton follow-up chains are supported when predicates are driven by earlier singleton roll-option selections. Same-item class-choice predicates are supported for earlier class-choice roll options on the same source item; broader actor-roll-option predicates still need content-driven audit before being called broadly covered.
...
- Battle Creed, Way of the Spellshot, Palatine Detective, Bloodrager, and Vindicator have registered scope through level 5. Bloodrager/Vindicator Free Archetype combinations, progression from level 6 onward, and optional Vindicator domain-spell feats are unqualified. The two new profiles require Free Archetype disabled for the guided scope; combining them with that variant requires a GM decision and manual PF2E setup because their mandatory dedication engages the normal dedication lockout. The [other eight profiles](../roadmap.md#parallel-breadth--class-archetypes) remain intentionally hidden until their replaced features, forced feats, nested choices, and subsystem changes have dedicated evidence.
...
8. Keep Starting Equipment on its prepared acquisition seam; do not widen it into daily preparations, ongoing purchasing, additive existing-inventory flows, or AP-specific story context.
