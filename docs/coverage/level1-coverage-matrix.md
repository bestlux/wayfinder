...
- Static source audit: `D:\Source\pf2e\packs\pf2e`, scoped to `Pathfinder Player Core` and `Pathfinder Player Core 2`.
- Scratch evidence: `.tmp/level1-launch-audit-summary.json`.
...
| Default path | Level-1 skill feat filtering | Drafted training gives skill and lore ranks | Open skill-feat picker | Supported `trained in ...` prerequisites filter against projected ranks | `tests/pack-service.test.ts`, `tests/wayfinder-option-context-service.test.ts` | merge |
...
| Default path | Other PC/PC2 spellcasting classes | Bard, druid, witch, oracle, sorcerer, champion | Select class | Non-spell branch and training choices can be discovered, but class-specific spell selection is not broadly guided | Static audit plus contributor registry review | release note / deferred |
...
- Predicate-gated static UUID grants are supported for item predicates, actor ancestry/class predicates, actor skill-rank predicates, and active roll options created by drafted rule selections. Selected-item or equipment-derived predicates, such as Weapon Innovation weapon category gates, still need a dedicated context slice.
...
5. Broaden option-context predicates for selected items and equipment-derived roll options before guiding Weapon Innovation, Armor Innovation, and similar side-book paths.
6. Add a generic config catalog resolver only if a real creation blocker such as `Samsaran Weapon Memory` is worth guiding.
