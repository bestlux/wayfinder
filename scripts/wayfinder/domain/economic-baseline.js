...
export function evaluateEconomicAdmission(args) {
    ...
    const nativeGrantIds = new Set(args.preparedClassGrantPlan.grants
        .filter((grant) => grant.materializer === "pf2e-native")
        .map((grant) => grant.grantId));
    const ignoredClassGrantItemIds = new Set(reconciliation.entries.flatMap((entry) => entry.status === "resolved" && nativeGrantIds.has(entry.grantId) ? entry.itemIds : []));
    ...
    for (const item of baseline.physicalItems) {
        ...
        const expectedContainerOwners = expected?.plannedContainerId
            ? baseline.physicalItems.filter((candidate) => retryEntries.get(candidate.acquisitionIdentity?.plannedItemId ?? "")?.ownedContainerId ===
