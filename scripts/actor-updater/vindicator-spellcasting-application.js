...
export function vindicatorSpellcastingSourceSelections(actor, draft) {
    const items = listActorItems(actor);
    if (!hasVindicatorProfile(draft, items))
        return [];
...
export async function syncVindicatorSpellcasting(actor, draft, sourceFactory) {
    const [selection] = vindicatorSpellcastingSourceSelections(actor, draft);
    if (!selection)
        return;
    const spells = listActorItems(actor).filter((item) => item.type === "spell");
    ...
    if (!entry?.id)
        throw new Error("Cannot prepare Vindicator spellcasting: its focus entry was not created.");
    if (source) {
        source.system ??= {};
    ...
    const entries = listActorItems(actor).filter((item) => item.type === "spellcastingEntry");
    const updates = spells.flatMap((spell) => {
        if (!isVindicatorWardenSpell(spell))
            return [];
        ...
        if (currentEntry && isDivineWisdomFocusEntry(currentEntry))
            return [];
...
function assertUnambiguousVindicatorSpellcasting(items) {
    ...
    if (items.filter((item) => item.type === "spellcastingEntry" && item.flags?.[MODULE_ID]?.destinationKey === DESTINATION_KEY).length > 1) {
        throw new Error("Cannot prepare Vindicator spellcasting: duplicate managed focus entries exist.");
    }
}
async function ensureVindicatorEntry(actor, mark) {
    const entries = listActorItems(actor).filter((item) => item.type === "spellcastingEntry");
    ...
    if (existing && isDivineWisdomFocusEntry(existing))
        return existing;
    const [created] = (await actor.createEmbeddedDocuments?.("Item", [desiredSource])) ?? [];
    return created ?? null;
}
function isDivineWisdomFocusEntry(item) {
...
function isVindicatorWardenSpell(item) {
    const traits = item.system?.traits?.value;
    return (itemMatchesSourceId(item, VINDICATOR_MARK_UUID) ||
        (Array.isArray(traits) && traits.includes("focus") && traits.includes("ranger")));
}
function createVindicatorEntrySource() {
    return {
...
//# sourceMappingURL=vindicator-spellcasting-application.js.map