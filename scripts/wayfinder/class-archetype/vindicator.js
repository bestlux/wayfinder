...
export function hasVindicatorProfile(draft, actorItems) {
...
export function vindicatorTracklessSelection() {
    return {
...
export function readVindicatorTracklessTerrain(actorItems) {
    const values = actorItems
        .filter((item) => itemMatchesSourceId(item, TRACKLESS_JOURNEY_UUID))
        .map((item) => item.flags?.[MODULE_ID]?.[VINDICATOR_TRACKLESS_FLAG]);
...
export function buildVindicatorTracklessJourneySteps(args) {
    if (args.targetLevel < 5 || !hasVindicatorProfile(args.draft, args.actorItems))
        return [];
    if (!args.draft.classChoices[VINDICATOR_TRACKLESS_SLOT] && readVindicatorTracklessTerrain(args.actorItems))
        return [];
    const selection = vindicatorTracklessSelection();
    return [
        createClassChoiceStep(5, {
            slotId: VINDICATOR_TRACKLESS_SLOT,
...
export function assertVindicatorTracklessChoice(step, value, source, activeProfileValue, targetLevel, actorItems = []) {
    const meta = step.classChoice;
    if (activeProfileValue !== "vindicator" ||
        targetLevel < 5 ||
        ...
        throw new Error("Cannot persist Trackless Journey terrain: its Vindicator profile or feature authority has changed.");
    }
}
export function isVindicatorTracklessMeta(meta) {
    return (meta?.profileChoice === "vindicator-trackless-journey" &&
...
export function materializeVindicatorTracklessTerrain(source, terrain) {
    source.flags ??= {};
...
function isVindicatorTerrain(value) {
    return value === "urban" || value === "natural";
}
//# sourceMappingURL=vindicator.js.map