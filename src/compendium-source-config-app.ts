...
interface CompendiumSourceContext {
  rows: CompendiumRowView[];
  filters: FilterView[];
  sorts: FilterView[];
  searchTerm: string;
  ...
  hasUnmatchedLegacyPatterns: boolean;
  hasGlobalWildcard: boolean;
  legacyPatterns: string;
  unmatchedLegacyPatterns: string;
}
...
export class CompendiumSourceConfigApp extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  #rows: CompendiumCatalogRow[];
  #selectedIds: Set<string>;
  #unavailableIds: Set<string>;
  #legacyPatterns: string[];
  #unmatchedLegacyPatterns: string[];
  #preservedLegacyPatterns: Set<string>;
  #hasGlobalWildcard: boolean;
  #scanGeneration = 0;
  #scanning = false;
  #scanStarted = false;
  ...
  constructor(options: Record<string, unknown> = {}) {
    ...
    const migration = resolveCompendiumSelection(
      getExtraPackSetting(),
    ...
    for (const id of this.#unavailableIds) this.#selectedIds.add(id);
  ...
  protected async _prepareContext(): Promise<CompendiumSourceContext> {
  ...
  async _onRender(context: CompendiumSourceContext, options: Record<string, unknown>): Promise<void> {
    await super._onRender(context, options);
    const root = this.element;
    if (!(root instanceof HTMLElement)) return;

    root.querySelector<HTMLInputElement>("[data-source-search]")?.addEventListener("input", (event) => {
      this.#searchTerm = (event.currentTarget as HTMLInputElement).value;
      this.#applyClientFilter(root);
    });
    root.querySelector<HTMLSelectElement>("[data-source-content-filter]")?.addEventListener("change", (event) => {
      this.#contentFilter = (event.currentTarget as HTMLSelectElement).value;
      this.#applyClientFilter(root);
    });
    root.querySelector<HTMLSelectElement>("[data-source-sort]")?.addEventListener("change", (event) => {
      this.#sortOrder = (event.currentTarget as HTMLSelectElement).value;
      this.render(false);
    });
    root.querySelector<HTMLInputElement>("[data-source-selected-only]")?.addEventListener("change", (event) => {
      this.#selectedOnly = (event.currentTarget as HTMLInputElement).checked;
      this.#applyClientFilter(root);
    });
    for (const checkbox of root.querySelectorAll<HTMLInputElement>("[data-source-pack-id]")) {
      checkbox.addEventListener("change", () => {
        const packId = checkbox.dataset.sourcePackId;
        if (!packId) return;
        if (checkbox.dataset.sourceLegacyPattern === "true") {
          if (checkbox.checked) this.#preservedLegacyPatterns.add(packId);
          else this.#preservedLegacyPatterns.delete(packId);
          this.#patchSelectedSummary(root);
          this.#applyClientFilter(root);
          return;
        }
        if (checkbox.checked) this.#selectedIds.add(packId);
        else {
    ...
    root.querySelector<HTMLElement>("[data-action='select-visible']")?.addEventListener("click", () => {
      this.#setVisibleSelection(root, true);
    });
    root.querySelector<HTMLElement>("[data-action='clear-visible']")?.addEventListener("click", () => {
      this.#setVisibleSelection(root, false);
    });
    root.querySelector<HTMLElement>("[data-action='refresh-counts']")?.addEventListener("click", () => {
      this.#refreshCounts();
    });
  ...
  static async #handleSubmit(
    this: CompendiumSourceConfigApp,
    _event: SubmitEvent,
    ...
    const stored = serializeCompendiumSelection(
      Array.from(this.#selectedIds).filter((id) => allowedIds.has(id)),
      this.#preservedLegacyPatterns
    );
  ...
  #buildRowViews(): CompendiumRowView[] {
    ...
    const unmatchedPatterns = this.#unmatchedLegacyPatterns.map(
      (pattern): CompendiumRowView => ({
        id: pattern,
        title: pattern,
    ...
    return [...available, ...unavailable, ...unmatchedPatterns].sort((left, right) =>
      this.#compareRowViews(left, right)
    );
  }

  #rowView(row: CompendiumCatalogRow): CompendiumRowView {
    const badges: CompendiumBadgeView[] = [];
    ...
    for (const pattern of this.#unmatchedLegacyPatterns) {
      ...
      if (this.#preservedLegacyPatterns.has(pattern)) {
        for (const id of refreshed.selectedIds) {
          if (!isOfficialCompendium(id)) this.#selectedIds.add(id);
        }
      }
  ...
  #setVisibleSelection(root: HTMLElement, selected: boolean): void {
    for (const checkbox of root.querySelectorAll<HTMLInputElement>("[data-source-pack-id]")) {
      const row = checkbox.closest<HTMLElement>("[data-source-row]");
      const packId = checkbox.dataset.sourcePackId;
      if (!packId || checkbox.disabled || row?.hidden) continue;
      checkbox.checked = selected;
      if (checkbox.dataset.sourceLegacyPattern === "true") {
        if (selected) this.#preservedLegacyPatterns.add(packId);
        else this.#preservedLegacyPatterns.delete(packId);
      } else if (selected) this.#selectedIds.add(packId);
      else {
        this.#selectedIds.delete(packId);
        this.#unavailableIds.delete(packId);
      }
  ...
  #applyClientFilter(root: HTMLElement): void {
    const query = this.#searchTerm.trim().toLowerCase();
    for (const row of root.querySelectorAll<HTMLElement>("[data-source-row]")) {
      const checkbox = row.querySelector<HTMLInputElement>("[data-source-pack-id]");
  ...
  #matchesContentFilter(row: HTMLElement): boolean {
    if (this.#contentFilter === "all") return true;
    if (this.#contentFilter === "relevant") return Number(row.dataset.sourceRelevant ?? 0) > 0;
    const kinds = (row.dataset.sourceKinds ?? "").split(" ");
    if (this.#contentFilter === "ancestry") return kinds.includes("ancestry") || kinds.includes("heritage");
    return kinds.includes(this.#contentFilter);
  }

  #compareRowViews(left: CompendiumRowView, right: CompendiumRowView): number {
    if (left.official !== right.official) return left.official ? -1 : 1;
    if (this.#sortOrder === "selected" && left.selected !== right.selected) return left.selected ? -1 : 1;
  ...
  #patchSelectedSummary(root: HTMLElement): void {
    const selectedCount = Array.from(root.querySelectorAll<HTMLInputElement>("[data-source-pack-id]")).filter(
      (checkbox) => checkbox.checked && !checkbox.disabled
    ).length;
    const target = root.querySelector<HTMLElement>("[data-source-selected-count]");
    if (target) target.textContent = String(selectedCount);
  }
}
