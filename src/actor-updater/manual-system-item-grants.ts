...
import type { ActorItemLike, EmbeddedItemSource, LooseRecord } from "../shared/actor-model.js";
...
export interface ManualSystemItemGrant {
  key: string;
  uuid: string;
  name: string;
...
export function stripManualSystemItemGrants(source: EmbeddedItemSource): void {
...
export function readManualSystemItemGrants(item: ActorItemLike): ManualSystemItemGrant[] {
...
export function selectionFromSystemGrant(grant: ManualSystemItemGrant): SelectionRef {
...
export function grantSourceMatches(item: ActorItemLike, uuid: string): boolean {
  return itemMatchesSourceId(item, uuid) || (CLAN_DAGGER_FEATURE_SOURCE_IDS.has(uuid) && item.name === "Clan Dagger");
}

export function applyManualGrantChoices(source: EmbeddedItemSource, choices: Record<string, string>): void {
