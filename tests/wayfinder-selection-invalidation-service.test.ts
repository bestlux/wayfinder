...
describe("wayfinder selection invalidation service", () => {
  it("clears a class selection and invalidates dependent draft prefixes", () => {
    ...
    draft.selections["ancestry-feat-level-1"] = {
      ...selection("ancestry-feat-level-1", "feat", "adapted-cantrip"),
      featType: "ancestry",
    };
    draft.selections["campaign-feat-ancestry-paragon-level-1"] = {
      ...selection("campaign-feat-ancestry-paragon-level-1", "feat", "adapted-cantrip"),
      featType: "ancestry",
    };
    ...
    const service = createSelectionInvalidationService(
      {
        ...
        pickerFiltersByStepId: new Map([[SLOT_IDS.class, { levelRange: null, rarity: ["common"], source: [] }]]),
    ...
    expect(service.clearSelection(SLOT_IDS.class)).toBe(10);
  ...
  it("clears filter-only dependent class steps when the class selection is cleared", () => {
    ...
    const pickerFiltersByStepId = new Map([
      ["class-branch-arcane-school-level-1", { levelRange: null, rarity: ["common"], source: [] }],
    ]);
    ...
    const service = createSelectionInvalidationService(
      {
        ...
        pickerFiltersByStepId: new Map([
          ["grant-choice-none-feat-molten-wit-feat-level-1", { levelRange: null, rarity: [], source: [] }],
        ]),
        recentlyInvalidatedStepIds: new Set<string>(),
