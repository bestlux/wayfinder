...
| Class branches | Guided when PF2E rule data is structured | Branch-discovery and branch-selection steps | Works when the class exposes selector-style branch rules cleanly. Class archetypes stay filtered. Options in predicate-backed branch steps stay visible as before; options in tag-based branch steps are filtered unless every embedded `ChoiceSet` is covered by an existing guided lane. |
