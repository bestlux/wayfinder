...
const localizedCatalogueInvariantCache = new WeakMap<
  StartingEquipmentCatalogueRecord,
  WeakMap<AcquisitionLocalize, Map<string, LocalizedCatalogueInvariant>>
>();
...
export function buildStartingEquipmentPane(
  ...
  const pendingExceptionSourceUuids = new Set(
    draft.equipmentPolicyRequests.flatMap((request) =>
  ...
  const projectRecord = (record: StartingEquipmentCatalogueRecord, index: number): CataloguePaneRow => {
    ...
    if (cachedRow?.volatileKey === volatileKey) return cachedRow.row;
    ...
    const noFundingReason =
      (record.pricePending ? localize("wayfinder-pf2e.StartingEquipment.Catalogue.ViewForPrice") : unavailableReason) ??
...
function cataloguePriceLabel(record: StartingEquipmentCatalogueRecord, localize: AcquisitionLocalize): string {
  if (record.pricePending) return localize("wayfinder-pf2e.StartingEquipment.Catalogue.ViewForPrice");
