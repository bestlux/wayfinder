...
export interface FeedbackVersions {
  wayfinder: string;
  foundry: string;
  pf2e: string;
}
 ...
 */
export function buildBugReportUrl(versions: Partial<FeedbackVersions>): string {
  const params = new URLSearchParams({ template: "bug-report.yml" });
  const prefill: Array<[string, string | undefined]> = [
