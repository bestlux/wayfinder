...
describe("starting equipment pane", () => {
  it("renders level-5 allowance buckets separately from residual coin", () => {
    ...
    const pane = buildStartingEquipmentPane(
      ...
      { state: "incomplete", complete: false, status: "Review purchases", issue: null },
      {
        ...
        recordSource: equipmentCatalogueRecordSourceFromRecords([record]),
        filters: [],
        activeFilters: {},
  ...
  it("requires an explicit eligible Titan Mauler choice before equipment review", () => {
    ...
    const pane = buildStartingEquipmentPane(
      ...
      { state: "incomplete", complete: false, status: "Choose a Titan Mauler weapon", issue: null },
      {
        ...
        query: "",
        matchedRecordCount: 1,
        recordSource: equipmentCatalogueRecordSourceFromRecords([weapon]),
        filters: [],
        activeFilters: {},
        previewSourceUuid: null,
  ...
  it("preserves the bounded projection window while retaining the total match count", () => {
    ...
    const pane = buildStartingEquipmentPane(
      ...
      { state: "incomplete", complete: false, status: "Review purchases or retain all", issue: null },
      {
        ...
        recordSource: equipmentCatalogueRecordSourceFromRecords(
          records.slice(0, STARTING_EQUIPMENT_RESULT_WINDOW.baselineSize)
        ),
        filters: [],
        activeFilters: {},
        previewSourceUuid: null,
  ...
  it("defers localization for thousand-row catalogues to exact mounted indexes and preserves facet order", () => {
    ...
    const projection = (ordered: readonly StartingEquipmentCatalogueRecord[], rowOrderKey: string) => ({
      ...
      recordSource: equipmentCatalogueRecordSourceFromRecords(ordered),
      filters: [],
      activeFilters: {},
      previewSourceUuid: null,
      titanMauler: { required: false, selectedSourceUuid: null },
  ...
  it("reuses localized row invariants while updating only changed funding and preview state", () => {
    ...
    const projection = (previewSourceUuid: string | null) => ({
      ...
      recordSource: equipmentCatalogueRecordSourceFromRecords(records),
      filters: [],
      activeFilters: {},
      previewSourceUuid,
      titanMauler: { required: false, selectedSourceUuid: null },
