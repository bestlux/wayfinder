...
interface AcquisitionSmokeCaseDefinition {
  ...
  readonly acquisitionCase: {
    ...
    readonly nativeGrant: Record<string, unknown> | null;
    readonly expectedAcquisitionItemCreateCheckpoints: number | null;
...
interface AcquisitionSmokeUiEvidence {
  ...
  acquisitionItemCreateCheckpoints: number;
}
...
export class AcquisitionSmokeCheckpointController {
  ...
  get initialItemCreateCheckpoints(): number {
  ...
  try {
    ...
    if (binding.checkpointTarget) {
      ...
      const recovery = await waitForValue(
        () => visibleRecoveryEvidence(actor, wayfinderApplication!, caseDefinition),
    ...
    assertExpectedItemCreateCheckpoints(caseDefinition, ui.acquisitionItemCreateCheckpoints);
    await waitForValue(
      () => (wayfinderRoot(wayfinderApplication!)?.isConnected !== true ? true : null),
...
async function reviewDisposition(
  ...
  if (acquisition.disposition === "purchase-ledger") {
    ...
    const add = await waitForValue(() => {
      ...
      const candidate = detail?.querySelector<HTMLElement>(
        `[data-wayfinder-action="add-equipment-item"][data-source-uuid="${DAGGER_SOURCE_UUID}"][data-funding="currency"]`
      );
      if (!selectedPreview?.isConnected || !detail?.isConnected || !candidate?.isConnected) return null;
      if (isDisabled(candidate)) throw new Error("The exact Dagger currency action is disabled.");
      return candidate;
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = globalThis.setTimeout(() => {
      ...
      const notifications = [...document.querySelectorAll<HTMLElement>(".notification")]
        ...
        .join(" | ");
      const statuses = [...document.querySelectorAll<HTMLElement>(".wayfinder-app .status-note span")]
        .map((entry) => entry.textContent?.trim())
        .filter(nonEmptyString)
        .join(" | ");
      reject(
        new Error(
          `The real Foundry Apply confirmation dialog did not render.${notifications ? ` Notifications: ${notifications}` : ""}${statuses ? ` Status: ${statuses}` : ""}`
        )
      );
    }, DRIVER_TIMEOUT_MS);
  });
...
async function exposeExpectedItemOnActorSheet(
  actor: SmokeActor,
  batchId: string,
  caseDefinition: AcquisitionSmokeCaseDefinition
): Promise<boolean> {
...
function visibleRecoveryEvidence(
  ...
  if (!status || !hasRecovery || !expectedLineVisible || !apply || apply.disabled) return null;
...
function assertReviewedAcquisitionDraft(
  ...
  if (expected.disposition === "retain-all") {
    ...
    if (
      acquisition.disposition.kind !== "retain-all" ||
...
function normalizeNativeGrant(
  ...
  const profile = NATIVE_GRANT_PROFILES[value.profileId as keyof typeof NATIVE_GRANT_PROFILES];
...
function assertCleanSmokeActor(actor: SmokeActor, caseDefinition: AcquisitionSmokeCaseDefinition): void {
  const state = normalizeDraftRecord(actor.getFlag(MODULE_ID, "state"));
  const draft = normalizeDraftRecord(actor.getFlag(MODULE_ID, "draft"));
...
function nativeFixtureDraftMatches(
  draft: Record<string, unknown> | null,
  nativeGrant: Record<string, unknown>
): boolean {
...
function applyCanRun(application: Record<string, unknown>, apply: HTMLButtonElement): boolean {
  const savePhase =
    wayfinderRoot(application)?.querySelector<HTMLElement>("[data-wayfinder-save-status]")?.dataset.phase;
  return (
    !apply.disabled &&
...
function cartLineByName(application: Record<string, unknown>, name: string): HTMLElement | null {
  return (
    [...(wayfinderRoot(application)?.querySelectorAll<HTMLElement>(".equipment-cart-line") ?? [])].find(
      (line) => line.querySelector("span strong")?.textContent?.trim() === name
    ) ?? null
  );
}
...
function assertExpectedItemCreateCheckpoints(caseDefinition: AcquisitionSmokeCaseDefinition, observed: number): void {
