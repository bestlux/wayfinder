...
export interface EquipmentCatalogueFilterProjection {
  readonly matchedEntries: readonly EquipmentCatalogueEntry[];
  readonly facets: readonly EquipmentCatalogueFacetOption[];
  readonly levelFacet: EquipmentCatalogueLevelFacet | null;
}
...
const FACET_KEYS = Object.freeze(["availability", "type", "rarity", "source", "trait"] as const);

type ProjectedFacetKey = (typeof FACET_KEYS)[number] | "titan-mauler";

interface EquipmentCatalogueEntryFilterFacts {
  readonly available: boolean;
  readonly itemType: string;
  readonly level: number;
  readonly publicationSlug: string;
  readonly rarity: string;
  readonly searchable: string | null;
  readonly titanMaulerEligible: boolean;
  readonly traits: readonly string[];
}

const IMMUTABLE_ENTRY_FILTER_FACTS = new WeakMap<EquipmentCatalogueEntry, EquipmentCatalogueEntryFilterFacts>();
...
export function projectEquipmentCatalogueFilters(input: {
  ...
  readonly selectedValues?: EquipmentCatalogueFilterMap;
  readonly includeTitanMaulerFacet?: boolean;
}): EquipmentCatalogueFilterProjection {
  const { entries, filters } = input;
  const selectedValues = input.selectedValues ?? {};
  const projectedKeys: readonly ProjectedFacetKey[] = input.includeTitanMaulerFacet
    ? [...FACET_KEYS, "titan-mauler"]
    : FACET_KEYS;
  const valuesByKey = new Map<ProjectedFacetKey, Set<string>>();
  const countsByKey = new Map<ProjectedFacetKey, Map<string, number>>();
  const selectedByKey = new Map<ProjectedFacetKey, ReadonlySet<string>>();
  for (const key of projectedKeys) {
    ...
    if (key === "titan-mauler") values.add("eligible");
    valuesByKey.set(key, values);
    countsByKey.set(key, new Map());
    selectedByKey.set(key, selected);
  }

  const matchedEntries: EquipmentCatalogueEntry[] = [];
  const allLevels = new Set<number>();
  const contextualLevels = new Set<number>();
  for (const entry of entries) {
    ...
    for (const trait of facts.traits) valuesByKey.get("trait")?.add(trait);

    const failures = equipmentCatalogueFilterFailures(facts, filters);
    if (failures === 0) matchedEntries.push(entry);
    if ((failures & ~FILTER_FAILURE.level) === 0) contextualLevels.add(facts.level);
    ...
    for (const trait of facts.traits) countProjectedFacetValue("trait", trait, failures);
  ...
  const facets = projectedKeys.flatMap((key) => {
    ...
    const selected = selectedByKey.get(key) ?? new Set<string>();
    return [...values]
      .map((value) => ({
        key,
        value,
      ...
      .filter(
        (option) => option.count > 0 || selected.has(option.value) || key === "availability" || key === "titan-mauler"
      )
  ...
  function countProjectedFacetValue(key: ProjectedFacetKey, value: string, failures: number): void {
    const excludedFailure = filterFailureForFacet(key);
    if ((failures & ~excludedFailure) !== 0) return;
...
export function rankEquipmentCatalogueMatches(
  entries: readonly EquipmentCatalogueEntry[],
  query: string
): EquipmentCatalogueEntry[] {
  const normalizedQuery = query.trim().toLocaleLowerCase();
  if (!normalizedQuery) {
    const available: EquipmentCatalogueEntry[] = [];
    const unavailable: EquipmentCatalogueEntry[] = [];
    for (const entry of entries) (entry.available ? available : unavailable).push(entry);
...
function buildEquipmentCatalogueLevelFacetFromValues(
  allLevelValues: ReadonlySet<number>,
  contextualLevelValues: ReadonlySet<number>,
  requested: NormalizedEquipmentCatalogueFilters["levelRange"]
): EquipmentCatalogueLevelFacet | null {
...
function equipmentCatalogueEntryFilterFacts(
  entry: EquipmentCatalogueEntry,
  includeSearchText: boolean
): EquipmentCatalogueEntryFilterFacts {
  const cacheable =
  ...
  if (cacheable) IMMUTABLE_ENTRY_FILTER_FACTS.set(entry, facts);
  return facts;
}

function equipmentCatalogueFilterFailures(
  facts: EquipmentCatalogueEntryFilterFacts,
  filters: NormalizedEquipmentCatalogueFilters
): number {
  let failures = 0;
  if (filters.policyAvailable && !facts.available) failures |= FILTER_FAILURE.availability;
  if (filters.titanMaulerEligible && !facts.titanMaulerEligible) failures |= FILTER_FAILURE.titanMauler;
  if (filters.itemTypes.size > 0 && !filters.itemTypes.has(facts.itemType)) failures |= FILTER_FAILURE.type;
  if (filters.rarities.size > 0 && !filters.rarities.has(facts.rarity)) failures |= FILTER_FAILURE.rarity;
...
function filterFailureForFacet(key: ProjectedFacetKey): number {
  switch (key) {
    case "availability":
...
function catalogueQueryRelevance(entry: EquipmentCatalogueEntry, normalizedQuery: string): number {
