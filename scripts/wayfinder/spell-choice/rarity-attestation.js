...
import { canGrantRestrictedSpellRarityAccess, SPELL_RARITY_CEILINGS, spellChoiceRarityCeiling, } from "./rarity-access.js";
const MAX_REASON_LENGTH = 500;
const MAX_IDENTITY_LENGTH = 200;
export function createSpellRarityAttestation(args) {
    const subject = buildSpellRarityAttestationSubject(args.actorId, args.step, args.targetLevel, args.worldRarityCeiling);
...
export function migrateLegacySpellRarityAccess(slotId) {
    return {
        version: 1,
        kind: "spell-rarity-access",
        trust: "player-attestation",
...
export function normalizeSpellRarityAttestation(slotId, raw) {
    if (!slotId)
        return null;
    if (raw === true)
        return migrateLegacySpellRarityAccess(slotId);
...
export function normalizeAppliedSpellRarityAttestation(raw) {
    ...
    const selectedSpells = raw.selectedSpells.flatMap((selection) => {
...
export function evaluateSpellRarityAttestation(actorId, draft, step, worldRarityCeiling) {
...
export function listSpellRarityAttestationProblems(actorId, draft, steps, worldRarityCeiling) {
            ...
            return [
                {
                    ...
                    message: "A restricted-spell player attestation no longer matches this character plan. Clear or rebuild the draft.",
                },
            ];
        }
...
export function listSpellRarityRecoveryProblems(actorId, draft) {
    const frozenBySlotId = new Map(draft.applySpellRarityAttestations.map((attestation) => [attestation.subject.slotId, attestation]));
    const problems = [];
    for (const [slotId, attestation] of Object.entries(draft.spellRarityAttestations)) {
        ...
        if (attestation.status !== "attested" ||
            ...
            problems.push({
                ...
                stepId: attestation.status === "attested" ? attestation.subject.stepId : slotId,
                title: "Restricted spell access",
                message: "The partial-Apply spell attestation receipt no longer matches the retained draft. Reopen Wayfinder or recover the actor manually.",
            });
        }
    }
    for (const frozen of draft.applySpellRarityAttestations) {
        if (!draft.spellRarityAttestations[frozen.subject.slotId]) {
            problems.push({
                ...
                message: "The partial-Apply spell attestation receipt has no matching retained claim. Reopen Wayfinder or recover the actor manually.",
            });
        }
    }
    return problems;
}
export function frozenSpellRarityAttestationForStep(actorId, draft, step) {
    const attestation = draft.spellRarityAttestations[step.slotId];
    if (attestation?.status !== "attested")
        return null;
    const frozen = draft.applySpellRarityAttestations.find((candidate) => candidate.subject.slotId === step.slotId && candidate.subject.stepId === step.id);
    return frozen && appliedAttestationMatchesDraft(actorId, draft, attestation, frozen) ? frozen : null;
}
export function buildAppliedSpellRarityAttestations(actorId, draft, steps, worldRarityCeiling) {
...
function appliedAttestationMatchesDraft(actorId, draft, attestation, frozen) {
    const frozenAttestation = normalizeSpellRarityAttestation(attestation.subject.slotId, frozen);
    return (frozenAttestation?.status === "attested" &&
...
export function removeOrphanedSpellRarityAttestations(draft, steps) {
...
export function buildSpellRarityAttestationSubject(actorId, step, targetLevel, worldRarityCeiling) {
...
export function spellRarityAttestationBasisLabel(basis) {
    return basis === "rules-access" ? "Character or rules Access" : "GM permission reported by player";
}
export function buildSpellRarityAttestationReviewLines(attestations) {
    return attestations.map((attestation) => {
        const spells = attestation.selectedSpells.map((spell) => spell.name).join(", ") || "no selected spells";
        return `Player attestation — not GM authorization: ${attestation.subjectLabel}; ${spellRarityAttestationBasisLabel(attestation.claimedBasis)}; ${spells}; recorded by ${attestation.authorName} at ${attestation.attestedAt}; reason: ${attestation.reason}`;
    });
}
...
function normalizeSelectionRef(raw, slotId) {
    if (!isRecord(raw) || raw.slotId !== slotId)
        return null;
    ...
    if (!packId || !documentId || !uuid || !itemType || !name)
        return null;
    if (raw.level !== null && !Number.isInteger(raw.level))
        return null;
    const slug = typeof raw.slug === "string" ? raw.slug : raw.slug === null ? null : undefined;
    return {
        slotId,
        packId,
        documentId,
...
function isSpellRarityCeiling(value) {
    return SPELL_RARITY_CEILINGS.includes(value);
}
...
//# sourceMappingURL=rarity-attestation.js.map